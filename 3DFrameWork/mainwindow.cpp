...
//	connect(action_saveas_, SIGNAL(triggered()), imagewidget_, SLOT(SaveAs()));
	...
	// Minimal Surface
	action_minimize_ = new QAction(tr("MinimalSurface"), this);
	action_minimize_->setStatusTip("Minimize the surface");
	connect(action_minimize_, SIGNAL(triggered()), this, SLOT(MinimizeSurface()));
}
...
void MainWindow::CreateToolBars()
{
	...
	// Minimal surface toolbar
	toolbar_minisurf_ = addToolBar(tr("MinimalSurface"));
	toolbar_minisurf_->addAction(action_minimize_);
}
...
void MainWindow::MinimizeSurface()
{
	if (!renderingwidget_->ptr_mesh_->isValid())
	{
		QMessageBox::about(this, "Mesh", "Mesh is not valid!");
		return;
	}

    bool ok;
	double offset = QInputDialog::getDouble(this,
		tr("Set the max offset"),
		tr("Please enter the max allowable error of distance,from 0.0001 to 1.0"),
		0.005, 0.0001, 1, 4, &ok);
	if (ok)
	{
        CMiniSurf mini(renderingwidget_->ptr_mesh_, (float)offset);
        renderingwidget_->updateGL();
	}
}
